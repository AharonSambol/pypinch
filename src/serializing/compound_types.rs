...
use crate::utils::consts::{
    BOOL_FLAG, CONSISTENT_TYPE_LIST_FLAG, DICT_FLAG, EMPTY_DICT_FLAG, EMPTY_LIST_FLAG,
    INVALID_UTF_8_START_BYTE_COMPACT_ASCII, LIST_FLAG, LIST_OF_STRUCTURED_DICTS_FLAG, NULL_FLAG,
    NUMBER_BASE, STR_KEY_DICT_FLAG,
};
use crate::utils::py_helpers::ToPyErr;
use crate::utils::wrappers::{
    get_list_size, get_tuple_size, is_ascii, list_get_item, py_unicode_data, tuple_get_item,
};
use pyo3_ffi::{
    PyBool_Type, PyDict_Next, PyDict_Size, PyDict_Type, PyList_Type, PyObject, PyTuple_Type,
    ...
    while unsafe { PyDict_Next(obj, &mut pos, &mut key, &mut val) } != 0 {
        unsafe {
            if (*key).ob_type == &mut PyTuple_Type {
                return Err(
                    "Invalid type for dict key: tuple".to_py_error(SERIALIZATION_ERROR_TYPE)
                );
            }
        }
...
fn is_consistent_type_list(obj: *mut PyObject, is_list: bool, len: Py_ssize_t) -> bool {
    let first_item = if is_list {
        list_get_item(obj, 0)
    } else {
        tuple_get_item(obj, 0)
    };
...
fn compare_dict_keys(dict: *mut PyObject, expected_keys: &Pointers) -> bool {
    ...
    if keys_count as usize != expected_keys.len() {
        return false;
    }
    while unsafe { PyDict_Next(dict, &mut pos, &mut key, &mut value) } != 0 {
