...
pub unsafe fn deserialize_object<'a>(
    ...
    *ptr += 1;
    match flag {
        ...
        FLOAT_FLAG => Ok(PyFloat_FromDouble(decode_f64(buf, ptr))),
        ...
        EMPTY_DICT_FLAG => Ok(PyDict_New()),
        EMPTY_LIST_FLAG => if use_tuples { Py_INCREF(EMPTY_TUPLE); Ok(EMPTY_TUPLE) } else { Ok(PyList_New(0)) },
        ...
        _ => Ok(PyLong_FromLong((flag - AMOUNT_OF_USED_FLAGS) as c_long)),
    }
}