...
#[inline(always)]
unsafe fn decode_str_key_dict<'a>(
    buf: &'a [u8],
    ptr: &mut usize,
    pointers: &mut FxHashMap<usize, *mut PyObject>,
    use_tuples: bool,
...
#[inline(always)]
unsafe fn decode_consistent_type_list<'a>(
    buf: &'a [u8],
    ptr: &mut usize,
    pointers: &mut FxHashMap<usize, *mut PyObject>,
    use_tuples: bool,
...
#[inline(always)]
unsafe fn decode_bytes(buf: &[u8], ptr: &mut usize) -> *mut PyObject {
    let len = decode_number__py_ssize_t::<NUMBER_BASE>(buf, ptr);
    let bytes = PyBytes_FromStringAndSize(
        buf.as_ptr().add(*ptr) as *const c_char,
        len,
...
#[inline(always)]
unsafe fn decode_pointer(buf: &[u8], ptr: &mut usize, pointers: &mut FxHashMap<usize, *mut PyObject>) -> *mut PyObject {
...
#[inline(always)]
unsafe fn decode_null() -> *mut PyObject {
    let none = Py_None();
    Py_INCREF(none);
    none
}
...
#[inline(always)]
unsafe fn decode_false() -> *mut PyObject {
    let f = Py_False();
    Py_INCREF(f);
    f
}
...
#[inline(always)]
unsafe fn decode_true() -> *mut PyObject {
    let t = Py_True();
    Py_INCREF(t);
    t
}
...
#[inline(always)]
unsafe fn decode_negative_int(buf: &[u8], ptr: &mut usize) -> *mut PyObject {
