...
unsafe fn decode_floats_list(buf: &[u8], ptr: &mut usize, len: Py_ssize_t) -> Result<*mut PyObject, *mut PyObject> {
...
unsafe fn decode_bytes_list(buf: &[u8], ptr: &mut usize, len: Py_ssize_t) -> Result<*mut PyObject, *mut PyObject> {
    let list = PyList_New(len);
    for i in 0..len {
...
unsafe fn decode_int_list(buf: &[u8], ptr: &mut usize, len: Py_ssize_t) -> Result<*mut PyObject, *mut PyObject> {
    let list = PyList_New(len);
    for i in 0..len {
